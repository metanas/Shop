	...
	public function addStockStatus($data) {
		foreach ($data['stock_status'] as $language_id => $value) {
			if (isset($stock_status_id)) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "stock_status (stock_status_id, language_id, name) VALUES('" . (int)$stock_status_id . "','" . (int)$language_id . "','" . $this->db->escape($value['name']) . "')");
			} else {
				$this->db->query("INSERT INTO " . DB_PREFIX . "stock_status (language_id, name) VALUES('" . (int)$language_id . "','" . $this->db->escape($value['name']) . "')");
