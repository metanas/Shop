	...
	public function addOrderStatus($data) {
		foreach ($data['order_status'] as $language_id => $value) {
			if (isset($order_status_id)) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_status ( order_status_id, language_id, name) VALUES('" . (int)$order_status_id . "','" . (int)$language_id . "','" . $this->db->escape($value['name']) . "')");
			} else {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_status (language_id, name) VALUES('" . (int)$language_id . "','" . $this->db->escape($value['name']) . "')");
