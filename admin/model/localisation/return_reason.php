	...
	public function addReturnReason($data) {
		foreach ($data['return_reason'] as $language_id => $value) {
			if (isset($return_reason_id)) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "return_reason (return_reason_id, language_id, name) VALUES('" . (int)$return_reason_id . "','" . (int)$language_id . "','" . $this->db->escape($value['name']) . "')");
			} else {
				$this->db->query("INSERT INTO " . DB_PREFIX . "return_reason (language_id, name) VALUES('" . (int)$language_id . "','" . $this->db->escape($value['name']) . "')");
