	...
	public function addReturnAction($data) {
		foreach ($data['return_action'] as $language_id => $value) {
			if (isset($return_action_id)) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "return_action (return_action_id,  language_id, name) VALUES('" . (int)$return_action_id . "','" . (int)$language_id . "','" . $this->db->escape($value['name']) . "')");
			} else {
				$this->db->query("INSERT INTO " . DB_PREFIX . "return_action (language_id, name) VALUES('" . (int)$language_id . "', '" . $this->db->escape($value['name']) . "')");
