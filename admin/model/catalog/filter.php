...
class ModelCatalogFilter extends Model {
	public function addFilter($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "filter_group` (sort_order) values('" . (int)$data['sort_order'] . "'");

		$filter_group_id = $this->db->getLastId();

		foreach ($data['filter_group_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "filter_group_description ( filter_group_id, language_id,name) values('" . (int)$filter_group_id . "','" . (int)$language_id . "','" . $this->db->escape($value['name']) . "')");
		}

		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "filter (filter_group_id,sort_order) VALUES('" . (int)$filter_group_id . "','" . (int)$filter['sort_order'] . "')");

				$filter_id = $this->db->getLastId();

				foreach ($filter['filter_description'] as $language_id => $filter_description) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter_description (filter_id, language_id, filter_group_id, name) VALUES('" . (int)$filter_id . "','" . (int)$language_id . "','" . (int)$filter_group_id . "','" . $this->db->escape($filter_description['name']) . "')");
				}
			}
		}
	...
	public function editFilter($filter_group_id, $data) {
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				if ($filter['filter_id']) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter (filter_id, filter_group_id,sort_order) VALUES('" . (int)$filter['filter_id'] . "','" . (int)$filter_group_id . "','" . (int)$filter['sort_order'] . "')");
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter (filter_group_id, sort_order) VALUES('" . (int)$filter_group_id . "','" . (int)$filter['sort_order'] . "')");
				}
