...
class ControllerMailRegister extends Controller
{
    public function index(&$route, &$args, &$output)
    {
        ...
        $data['text_approval'] = sprintf($this->language->get('text_approval'), $args[0]['firstname'] ,$this->url->link('account/login/verify', array("email" => $args[0]['email'], "login_token" => $token)), $this->url->link('account/login/verify', array("email" => $args[0]['email'], "login_token" => $token)));
