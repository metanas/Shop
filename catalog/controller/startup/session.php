...
class ControllerStartupSession extends Controller {
	public function index() {
		if (isset($this->request->get['route']) && substr((string)$this->request->get['route'], 0, 4) == 'api/') {
			$this->db->query("DELETE FROM `" . DB_PREFIX . "api_session` WHERE  date_modified + interval '1 hours' < NOW()");
					
			// Make sure the IP is allowed
			$api_query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "api a LEFT JOIN " . DB_PREFIX . "api_session ass ON (a.api_id = ass.api_id) LEFT JOIN " . DB_PREFIX . "api_ip ai ON (a.api_id = ai.api_id) WHERE a.status = '1' AND ass.session_id = '" . $this->db->escape((string)$this->request->get['api_token']) . "' AND ai.ip = '" . $this->db->escape((string)$this->request->server['REMOTE_ADDR']) . "'");
